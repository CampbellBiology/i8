...
const Graph = ({
                ...
                if (selectedGraphKey === "temperature" || selectedGraphKey === "humidity") {
                    ...
                    if (selectedDataAmountUnitKey === "average_data_amount") {
                        ...
                        let dataObj2 = {
                            ...
                            data: (dataAmountData[serialNumber]?.cumulativeTransmitDataArray).map((item) => {
                                const date = new Date(item.date);
    ...
    return (
        <React.Fragment >
            ...
            <Button onClick={resetZoomTrigger}
                color='gray'>
                reset zoom
            </Button>
...
const Sticker_ = ({
    ...
    const onMove = useCallback((e: MouseEvent) => {
        ...
        if (wrapRef.current.offsetTop - lastY >= (wrapRef.current?.parentElement?.offsetTop + parentY - wrapRef.current?.clientHeight) || wrapRef.current?.offsetTop - lastY <= wrapRef.current?.parentElement?.offsetTop) {
            return;
        }
        if (wrapRef.current.offsetLeft - lastX >= (parentX - wrapRef.current?.parentElement?.offsetLeft * 2 - wrapRef.current?.clientWidth / 2) || wrapRef.current.offsetLeft - lastX <= wrapRef.current?.parentElement?.offsetLeft) {
            return;
        }
    ...
    const getImageCoordinate = async () => {
        await axios.post("/api/device/image/coordinate", {
    ...
    return (
        ...
        >
            <div
                ...
                alt={serialNumber}
                className='absolute cursor-pointer flex'
            >
                <FlowbiteTooltip className='w-[150px] whitespace-pre'
...
const StickerTest = ({
    ...
    return (
        <React.Fragment>
            ...
            <div className='bg-gray-200 w-fit py-1 px-2 mb-3 rounded hover:cursor-pointer hover:brightness-90'
                onClick={() => setDeviceImageCoordinates()}>
                ✅ 기기 위치 저장
            </div>
            <div
                style={{ backgroundImage: `url('${deviceImageSrc}')`, height: '600px', width: '800px', backgroundSize: 'cover' }}
            >
                <div>
                    {
...
const CsvDownload = ({ tableId, filename }) => {
    const downloadCsv = () => {
                                            ...
                                            <Table.Row key={`${key}-${idx}-all-data-table`}>
                                                ...
                                                }
                                                {
                                                    tableReceiveStatus
                                                        ?
                                                        <Table.Cell>
                                                            {obj.receive_data_amount}
                                                        </Table.Cell>
                                                        :
                                                        null
                                                }
                                                <Table.Cell>
                                                    {`${String(date.getFullYear()).padStart(2, '0')}-${(String(date.getMonth() + 1)).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`} <br />
                                    ...
                                    <Table.Row
                                        ...
                                        {/* 송신량 */}
                                        {
                                            tableTransmitStatus
                                                ?
                                                <DataAmountTable
                                                    data={transmitDataAmountInfoBySerialNumber} /> : null
                                        }
                                        {/* 수신량 */}
                                        {
                                            tableReceiveStatus
                                                ?
                                                <DataAmountTable
                                                    data={receiveDataAmountInfoBySerialNumber} /> : null
                                        }
                                        {/* 송수신량 */}
                                        {
                                            tableTransmitAndReceiveStatus
                                                ?
                                                <DataAmountTable
                                                    data={allDataAmountInfoBySerialNumber} /> : null
                                        }
                                    </Table.Row>
                                )
                            })
...
export default function InfoPage() {
    ...
    async function triggerSearchGraphDataBySelectedDate() {
        ...
        if (startDate > endDate) {
            ...
            await Object.keys(deviceListDataCopy).map(async (key: string) => {
                ...
                const deviceListDataFiltered: any = deviceListDataKeyCopy.filter((item) => (
                ...
                let averagesResult: averagesResult = {
                    receive: { dailyAveragesResult: { sum: 0, count: 0 }, monthlyAveragesResult: { sum: 0, count: 0 }, yearAveragesResult: { sum: 0, count: 0 } },
                    transmit: { dailyAveragesResult: { sum: 0, count: 0 }, monthlyAveragesResult: { sum: 0, count: 0 }, yearAveragesResult: { sum: 0, count: 0 } }
                }

                // 일/월/연평균 최종계산
                // 수신
                await Object.keys(receive_averages.dailyAverages).map(async (key) => {
                    averagesResult.receive.dailyAveragesResult.sum += receive_averages.dailyAverages[key].sum / receive_averages.dailyAverages[key].count;
                    averagesResult.receive.dailyAveragesResult.count++;
                })
                await Object.keys(receive_averages.monthlyAverages).map(async (key) => {
                    averagesResult.receive.monthlyAveragesResult.sum += receive_averages.monthlyAverages[key].sum / receive_averages.monthlyAverages[key].count;
                    averagesResult.receive.monthlyAveragesResult.count++;
                })
                await Object.keys(receive_averages.yearAverages).map(async (key) => {
                    averagesResult.receive.yearAveragesResult.sum += receive_averages.yearAverages[key].sum / receive_averages.yearAverages[key].count;
                    averagesResult.receive.yearAveragesResult.count++;
                })

                // 송신
                await Object.keys(transmit_averages.dailyAverages).map(async (key) => {
                    averagesResult.transmit.dailyAveragesResult.sum += transmit_averages.dailyAverages[key].sum / transmit_averages.dailyAverages[key].count;
                    averagesResult.transmit.dailyAveragesResult.count++;
                })
                await Object.keys(transmit_averages.monthlyAverages).map(async (key) => {
                    averagesResult.transmit.monthlyAveragesResult.sum += transmit_averages.monthlyAverages[key].sum / transmit_averages.monthlyAverages[key].count;
                    averagesResult.transmit.monthlyAveragesResult.count++;
                })
                await Object.keys(transmit_averages.yearAverages).map(async (key) => {
                    averagesResult.transmit.yearAveragesResult.sum += transmit_averages.yearAverages[key].sum / transmit_averages.yearAverages[key].count;
                ...
                deviceDataAmountObjectCopy[key] = {
                    receive: {
                        ...
                        daily: averagesResult.receive.dailyAveragesResult.sum && !isNaN(averagesResult.receive.dailyAveragesResult.sum) ? averagesResult.receive.dailyAveragesResult.sum / averagesResult.receive.dailyAveragesResult.count : 0,
                        month: averagesResult.receive.monthlyAveragesResult.sum && !isNaN(averagesResult.receive.monthlyAveragesResult.sum) ? averagesResult.receive.monthlyAveragesResult.sum / averagesResult.receive.monthlyAveragesResult.count : 0,
                        year: averagesResult.receive.yearAveragesResult.sum && !isNaN(averagesResult.receive.yearAveragesResult.sum) ? averagesResult.receive.yearAveragesResult.sum / averagesResult.receive.yearAveragesResult.count : 0
                    },
                    transmit: {
                        all: sum_transmit && !isNaN(sum_transmit) ? sum_transmit : 0,
                        daily: averagesResult.transmit.dailyAveragesResult.sum && !isNaN(averagesResult.transmit.dailyAveragesResult.sum) ? averagesResult.transmit.dailyAveragesResult.sum / averagesResult.transmit.dailyAveragesResult.count : 0,
                        month: averagesResult.transmit.monthlyAveragesResult.sum && !isNaN(averagesResult.transmit.monthlyAveragesResult.sum) ? averagesResult.transmit.monthlyAveragesResult.sum / averagesResult.transmit.monthlyAveragesResult.count : 0,
                        year: averagesResult.transmit.yearAveragesResult.sum && !isNaN(averagesResult.transmit.yearAveragesResult.sum) ? averagesResult.transmit.yearAveragesResult.sum / averagesResult.transmit.yearAveragesResult.count : 0
                    }
                }
            })
            ...
            async function dateByUnitKey(existingData, d, deviceGraphDataFiltered, deviceGraphDataKeyCopy, key) {
                ...
                if (selectedGraphUnitKey === "min") {
                    existingData = await deviceGraphDataKeyCopy.filter((item: Device) =>
                        (
    ...
    const changeLocation = async () => {
        ...
        let tempObj: any = { selectedCity, selectedGu, selectedAddress, selectedGroup, selectedLocation }
        // console.log(tempObj)
        ...
        if (selectedGroup && selectedLocation) {
            ...
            await axios.post("/api/device/image", {
        ...
        <React.Fragment>
            ...
            <div className="flex items-start justify-center m-4">
                ...
                <div className="w-2/3 m-1">
                    <div className="flex items-start justify-between bg-gray-200 p-5">
                        {/* info */}
                        <div className="">
                            <div className="flex">
                                <b>설치 장소명</b> :
                                <div className="flex flex-col ml-3">
                                    <div className="flex">
                                        ...
                                        <SelectDropDown
                                            ...
                                            data={selectedGu ? reqGlobalRootState.findByNodeRecursion(selectedGu, reqGlobalRootState).children : []}
                                        />
                                    </div>
                                    <div className="flex">
                                        {/* 그룹 단위 */}
                                        <SelectDropDown
                                            ...
                                            data={selectedAddress ? reqGlobalRootState.findByNodeRecursion(selectedAddress, reqGlobalRootState).children : []}
                                        />
                                        {/* 장소 단위 */}
                                        <SelectDropDown
                                            ...
                                            setSelectedItemKey={setSelectedAddress}
                                            data={selectedGroup ? reqGlobalRootState.findByNodeRecursion(selectedGroup, reqGlobalRootState).children : []}
                                        />
                                    </div>
                    ...
                    <div className='flex p-3 justify-between'>
                        ...
                        <div className='flex items-center'>
                            ...
                            <SelectDropDown
                                ...
                                data={["온도 그래프", "습도 그래프", "데이터 사용량 그래프", "기기 위치", "누적/평균 데이터량 조회", "통합 데이터 조회"]}
                    ...
                    <div className='w-full flex items-center justify-start' style={{position: 'relative'}}>
                        ...
                        <div className='flex gap-3'>
                            <Button color='gray'
                                onClick={() => triggerSearchGraphDataBySelectedDate()}>
                                🔍 검색
                            </Button>
                        </div>                 
...

                        {
                            selectedGraph === "데이터 사용량 그래프"
                                ...
                                <div className='flex gap-3 p-3'>
                                    <div className="flex items-center">
                                        <input checked={selectedDataAmountUnitKey === "average_data_amount" ? true : false}
                                            onClick={() => setSelectedDataAmountUnitKey("average_data_amount")}
                                            id="average-data-amount" type="radio" value="" name="default-radio"
                                    ...
                                    <div className="flex items-center">
                                        <input checked={selectedDataAmountUnitKey === "accumulate_data_amount" ? true : false}
                                            onClick={() => setSelectedDataAmountUnitKey("accumulate_data_amount")}
                                            id="accumulate-data-amount" type="radio" value="" name="default-radio"
                                            className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 
                                            hover:cursor-pointer
                                            focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                                        <label for="accumulate-data-amount"
                                            className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                                            누적 사용량
                                        </label>
                                    </div>
                    ...
                    <div className='text-blue-500 mb-3 text-center text-sm'>
                        {searchGraphDataBySelectedDateStatus}
                    </div>
                            ...
                            selectedGraph === "누적/평균 데이터량 조회"
                                ...
                                selectedGraph === "기기 위치"
                                    ...
                                    selectedLocation
                                        ...
                                        />
                                        :
                                        <div className='w-[800px] h-[600px] bg-gray-300
                ...
                <div className="w-1/3 m-1">
                    <div className="bg-gray-200 py-3 px-4 mb-2 flex justify-between">
                        <div>
                            <b>기기 리스트</b>
                        </div>
                        <div>
                            총 기기 수 : {allDataNode ? allDataNode?.length : null}대
                        </div>
                    </div>

                    <div className="max-h-[920px] overflow-y-scroll">
                        {
                            allDataNode?.map((item: ClusterNode, index) => {
                                if (deviceDataByClusterKey) {
                                    let data = (deviceDataByClusterKey.filter((item_) => item_.product_serial_number === item.product_serial_number));
                                    ...
                                    if (currentData.length) {
                                        var discomfort: number = ((9 / 5) * currentData.temperature) - 0.55 * (1 - currentData.humidity / 100) * ((9 / 5) * currentData.temperature - 26) + 32;
                                ...
                                return (
                                    <>
                                        <div className="border border-1 border-gray-400 rounded-md p-3 my-2
                                    flex justify-between items-center">
                                            {/* description */}
                                            <div className='flex items-center'>
                                                <span
                                                    content="기기 선택은 5개까지 가능합니다.">
                                                    <input type="checkbox"
                                                        className='hover:cursor-pointer mr-3'
                                                        value={item.product_serial_number}
                                                        id={`device-${index}`}
                                                        onChange={e => onCheckedElement(e.target, e.target.checked, e.target.value)}></input>
                                                </span>
                                                <span>
                                                    <div>
                                                        <b>제품명</b>: I8_SENSOR
                                                    ...
                                                    discomfortSrc
                                                        ...
                                                        <React.Fragment>
                                                            ...
                                                            <div className='text-sm text-center'>
                                                                불쾌지수
                                                                <div className={`${discomfortColor} font-semibold`}>
                                                                    {discomfortString}
                                                                </div>
                                                            </div>
                                                        </React.Fragment>
