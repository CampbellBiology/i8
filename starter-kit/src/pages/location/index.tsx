...
export default function LocationPage() {
    ...
    const moveToInfoFromRoot = (node: ClusterNode) => {
        ...
        const queryObject = (selectedCity: string | null | undefined, selectedGu: string | null | undefined, selectedAddress: string | null, selectedGroup: string | null, selectedLocation: string | null) => {
            const queryObjectResult = {
                selectedCity, selectedGu, selectedAddress, selectedGroup, selectedLocation
            }
        ...
        if (depth === 3) {
            const parentsNode: ClusterNode | null = node.findParentNodeContainingNode(node, rootCopy);
            const parentsNode_: ClusterNode | null | undefined = parentsNode?.findParentNodeContainingNode(parentsNode, rootCopy);
                ...
                <div className="w-2/3 border border-1 border-black p-2">
                    ...
                    <div>
                        <div className="flex py-2 px-1 justify-start items-center">
                            <Button color="gray" onClick={() => setAddressOpen(true)}>주소 검색</Button>
                                            ...
                                            {({ open }: any) => (
                                                <div>
                                                    <AccordionHeader className="text-left">
                                                        <div  className={`border border-1 border-black rounded-md mx-1 my-2 p-2 bg-white`}
                                                                                                ...
                                                                                                {({ open }: any) => (
                                                                                                    <div>
                                                                                                        <AccordionHeader onClick={(e) => e.stopPropagation()}
                                                                                                            className="text-left">
                                                                                                            <div className="border border-1 border-black rounded-md mx-1 my-2 p-2 bg-white flex items-center justify-between"
                                                                                                                onClick={() => changeRoot(childRoot, item_?.name, true)}
                                                                                                                ...
                                                                                                                {/* arrow */}
                                                                                                                <div className="bg-gray-200 h-5 -mx-2 mt-1 -mb-2 -p-2 rounded-b-md 
